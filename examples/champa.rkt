...
(define (withdraw-owner! address) (withdraw! address (balance **instance**)))
...
(define (modify-balance! address f) (modify! balances address f))
...
(define (make-book) (book on-match))

(struct (book match-callback)
  ([ bids (sorted-map (on r cmp))      ]
   [ asks (sorted-map (on r cmp))      ]
   [ on-match (partial match-callback this) ]))

(define (book-bid? book user) (member? book user))
(define (book-bid! book user amount)
  (write! (at (book-bids book) user)
          amount)
  (try-cross-bid/ask! book))

(define (book-ask! book user amount)
  (write! (at (book-asks book) user)
          amount)
  (try-cross-bid/ask! book))
...
(define (book-pop-bid! book) (pop   (book-bids book)))
